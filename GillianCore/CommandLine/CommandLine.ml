...
struct
  ...
  module DebugVerificationConsole = struct
    ...
    let verify
      ...
      let () =
        process_files files true None false false
      in
      let () = if !Config.stats then Statistics.print_statistics () in
      Logging.wrap_up ()
    ...
    let debug_verify_info =
      ...
      let man =
        [
          `S Manpage.s_description;
          `P "Starts Gillian in debugging mode for verification, which \
    ...
    let start_debug_adapter () =
      Lwt_main.run (Debug_adapter.Adapter.start Lwt_io.stdin Lwt_io.stdout verify)
