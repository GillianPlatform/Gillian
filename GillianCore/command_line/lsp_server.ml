...
let loc_to_range (loc : Utils.Location.t) : Range.t = raise (Failure "TODO")
let default_range =
  let start = Position.create ~line:0 ~character:0 in
  let end_ = start in
  Range.create ~start ~end_

let mk_diagnostic ?loc ?(severity = DiagnosticSeverity.Error) (msg : string) : Diagnostic.t =
  let range = match loc with
  | Some loc -> loc_to_range loc
  | None -> default_range
  in
  Diagnostic.create ~severity ~range ~message:(`String msg) ()

let analysis_failure_to_diagnostic (failure : Gillian_result.Error.analysis_failure) : Diagnostic.t =
  raise (Failure "TODO")

let result_to_diagnostics: unit Gillian_result.t -> Diagnostic.t list = function
| Ok () -> []
| Error (AnalysisFailures failures) -> List.map analysis_failure_to_diagnostic failures
| Error (CompilationError { msg; loc; _ }) -> [ mk_diagnostic ?loc msg ]
| Error (OperationError msg | InternalError { msg; _ }) -> [ mk_diagnostic msg ]
  ...
  object (self)
    ...
    *)
    method private _on_doc
      ...
      let diags = result_to_diagnostics result in
