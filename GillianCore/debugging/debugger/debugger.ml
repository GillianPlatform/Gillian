  ...
  struct
    ...
    let unify result proc_name prev_id dbg =
      ...
      | Some test ->
          let+ id, content, success =
            ...
            | None ->
                DL.log (fun m ->
                    m "Unifying result for %a" L.ReportId.pp prev_id);
                let success =
      ...
      | Some cont_func -> (
          DL.log (fun m ->
              m
                ~json:
                  [
                    ("id", L.ReportId.to_yojson prev_id_in_frame);
          ...
          | EndOfBranch (result, cont_func) -> (
              ...
              let prev =
                let+ content, type_ =
                  L.LogQueryer.get_report prev_id_in_frame
                in
              ...
              | Some (prev_id, content, type_) when type_ = ContentType.cmd -> (
                  ...
                  let exec_data =
                    Lift.make_executed_cmd_data Exec_map.Final prev_id cmd
                      ~unifys ~errors branch_path
                  in
                  let handler_result =
                    state.lifter_state
                  ...
                  then (
                    ...
                    Exec_map.(
                      ...
                      let unifys =
                        let unify =
                          DL.log (fun m ->
                              m "getting unify_result for %a" L.ReportId.pp
                                cur_report_id);
                          let+ unify_id, _ =
                          ...
                          let unify_map =
                            match
                              state.unify_maps |> List.assoc_opt unify_id
                            with
                            | Some map -> map
                            | None ->
                                let map = build_unify_map unify_id in
                      ...
                      | ExecNext (id, branch_case) ->
                          DL.log (fun m ->
                              m "EXEC NEXT (%a, %a)" (pp_option L.ReportId.pp)
                                id (pp_option BranchCase.pp) branch_case);
    ...
    and launch_proc proc_name dbg =
      ...
      |> L.ReportState.with_state (fun () ->
             let cont_func =
               Verification.verify_up_to_procs ~init_data:cfg.init_data
                 ~proc_name cfg.prog
             in
             aux None cont_func)
      ...
      let stop_reason =
        if reverse then (
          ...
          let next_id =
            match
              state.lifter_state
              |> Lifter.existing_next_steps state.cur_report_id
            with
            | [] -> None
            | nexts -> (
