...
module Make (Impl : Impl.S) = struct
  ...
  let eval_laction x a es ctx =
    ...
    in
    cont_steps @ error_steps |> Choice.Indexed.make_const
    ...
    | SL (Invariant (a, binders)) ->
        ...
        let err_steps =
          ...
          |> List.map @@ fun err ->
             ...
             make_err ~errors ~callstack ctx
        in
        ok_steps @ err_steps |> Choice.Indexed.make_const
    | _ ->
        ...
        let error_steps =
          ...
          | _ ->
              ...
              [ make_err ~errors ~callstack ctx ]
        in
        success_steps @ error_steps |> Choice.Indexed.make_const
  ...
  let eval_guarded_goto e t_ix f_ix ctx =
    ...
    let&+ state, ix, callstack = states |> Choice.Indexed.make_const in
  ...
  let eval_cmd (ctx : step_ctx) : (int, step) Choice.t =
    match ctx.cmd with
