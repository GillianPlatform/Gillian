...
(** Given a strategy finds a predicate best matching it (int is non-null and
    highest possible). If consume is true, and the function returns [Some _],
...
(** [consume_pred ~maintain pred_state name args_opt ins f_eq] removes the
    predicate best matching the triple [(name, args_opt, ins)] using an equality
    function on values [f_eq]. It does so in place, and if maintain is true, the
    predicate is matched but not actually removed from the state. *)
val consume_pred :
  maintain:bool ->
  t ->
  string ->
