...
*)
  ...
  and evaluate_cmd_after_frame_handling
    ...
    | ReturnNormal ->
        ...
        let result =
          ...
          | Some v_ret, { store = None; loop_ids = start_loop_ids; _ } :: _ ->
              ...
              [
                ConfFinish
                  { flag = Normal; ret_val = v_ret; final_state = state };
              ]
          | ( Some v_ret,
              {
    ...
    | ReturnError -> (
        ...
        | Some v_ret, { store = None; loop_ids = start_loop_ids; _ } :: _ ->
            ...
            Fmt.pr "e @?";
            [
              ConfFinish { flag = Error; ret_val = v_ret; final_state = state };
            ]
        | ( Some v_ret,
            {
              store = Some old_store;
  ...
  let protected_evaluate_cmd
    ...
    List.concat_map
      (fun state ->
        try
          evaluate_cmd prog state cs iframes prev prev_loop_ids i b_counter report_id_ref branch_case
        with
  ...
  *)
  let rec evaluate_cmd_step
    ...
    Fun.protect ~finally:(fun () -> L.release_parent !report_id_ref) (fun () ->
      ...
      | ConfCont
          { state; callstack = cs; next_idx = i; branch_count = b_counter; prev_report_id; branch_case; _ }
        :: rest_confs ->
