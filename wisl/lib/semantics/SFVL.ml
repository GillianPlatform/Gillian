...
let t_of_yojson (yojson : Yojson.Safe.t) : t =
  let to_sfvl_list yojson_list =
    yojson_list
    |> List.map (fun (element : Yojson.Safe.t) ->
           match element with
           | `Tuple tuple ->
               if List.length tuple != 2 then
                 ...
                 let fn = Expr.t_of_yojson (List.hd tuple) in
                 let fv = Expr.t_of_yojson (List.nth tuple 1) in
                 (fn, fv)
           | _            ->
               failwith
  ...
  in
  match yojson with
  | `List list -> of_list (to_sfvl_list list)
  | _          -> failwith "Cannot parse yojson into WislSHeap: must be list"

let yojson_of_t (sfvl : t) : Yojson.Safe.t =
  `List
    (to_list sfvl
    |> List.map (fun (fn, fv) ->
           let fn = Expr.yojson_of_t fn in
           let fv = Expr.yojson_of_t fv in
           `Tuple [ fn; fv ]))
