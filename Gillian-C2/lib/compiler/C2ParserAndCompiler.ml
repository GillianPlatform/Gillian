...
(* let insert_logic_cmds : (Annot.t, string) Prog.t -> (Annot.t, string) Prog.t = failwith "TODO"
   let annotate : (Annot.t, string) Prog.t -> (Annot.t, string) Prog.t = failwith "TODO"

   let add_annots_to_program prog annots =
     prog
       |> insert_logic_cmds annots
       |> annotate annots *)
