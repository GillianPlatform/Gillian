...
let get_structs_not_annot ctx =
  let struct_names = ctx.prog.types
...
let gen_pred_of_struct ctx ann struct_name =
  let open Datatype_component in
  let pred_name = pred_name_of_struct struct_name in
  let pred_ins = [ 0; 1 ] in
  let components = Ctx.resolve_struct_components ctx (Ctx.tag_lookup ctx struct_name) in
  let first_params =
    [
      (loc_param_name, Some Type.ObjectType); (ofs_param_name, Some Type.IntType);
    ]
  in
  let struct_params : (string * GilType.t option) list =
    components
    |> List.filter_map @@ function
       | Field { name; _ } -> Some (name, Some Type.ListType)
       | Padding _ -> None
  in
  ...
  let def_without_holes =
    ...
    |> List.concat_map @@ function
       | Field { name; type_ } -> assert_of_member cenv comp.co_members id typ
       | Padding { name; bits } -> failwith "TODO"
  in
  let fo idp =
...
let trans_annots (ctx : Ctx.t) (log_prog : C2_prog.t) filepath =
  let open Ctx in
