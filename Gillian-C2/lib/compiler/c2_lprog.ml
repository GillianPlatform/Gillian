...
module CSVal = struct
  ...
  let size_of ~(ctx : Ctx.t) = function
    | Sint _ -> ctx.machine.int_width
    | Slong _ -> ctx.machine.long_int_width
    | Ssingle _ -> ctx.machine.single_width
    | Sfloat _ -> ctx.machine.double_width
    | Sptr _ | Sfunptr _ -> ctx.machine.pointer_width

  let chunk_of ~(ctx : Ctx.t) sval =
    let size = size_of ~ctx sval in
    let chunk =
      match sval with
      | Sint _ | Slong _ -> Chunk.of_int_type ~signed:true ~size
      | Ssingle _ | Sfloat _ -> Chunk.of_float_type ~size
      | Sptr _ | Sfunptr _ -> Chunk.of_int_type ~signed:false ~size
    in
    Option.get chunk

  let gil_type_of = function
    | Sint _ | Slong _ -> GilType.IntType
    | Ssingle _ | Sfloat _ -> GilType.NumberType
    | Sptr _ | Sfunptr _ -> GilType.ListType
end
...
module CAssert = struct
  ...
  let rec pp fmt a =
    ...
    | Array { ptr; chunk; size; content; malloced } ->
        ...
        Fmt.pf fmt "@[<h>%s(%a, %s, %a, %a)@]" pname CExpr.pp ptr
          (Chunk.to_string chunk) CExpr.pp size CExpr.pp content
    | Malloced (e1, e2) ->
