...
module Range = struct
  ...
  let equal ~pfs ~gamma (la, ha) (lb, hb) =
    let ( = ) = Gillian.Logic.FOSolver.is_equal ~pfs ~gamma in
    la = lb && ha = hb

  let inside ~pfs ~gamma (la, ha) (lb, hb) =
    let ( <= ) = Gillian.Logic.FOSolver.is_less_or_equal ~pfs ~gamma in
    lb <= la && ha <= hb
...
module Tree = struct
  ...
  let rec split ~pfs ~gamma ~range t =
    ...
    if ol === nl then
      ...
      let right_left, right_right = split ~pfs ~gamma ~range full_right in
      let right =
        with_children full_right ~left:right_left ~right:right_right
      in
      (left, right)

  let rec frame_single ~pfs ~gamma t low chunk =
    let range_eq = Range.equal ~pfs ~gamma in
    let range_ins = Range.inside ~pfs ~gamma in
    let range = Range.from_low_and_chunk low chunk in
    if range_eq range t.span then Ok t
    else
      match t.children with
      | Some (left, right) ->
          if range_ins range left.span then
            let+ left = frame_single ~pfs ~gamma left low chunk in
            with_children t ~left ~right
          else if range_ins range right.span then
            let+ right = frame_single ~pfs ~gamma right low chunk in
            with_children t ~left ~right
          else
            Error (Unhandled "Children don't seem to be a partition of parent")
      | None               ->
          let left, right = split ~pfs ~gamma ~range t in
          Ok (with_children t ~left ~right)

  let rec get_framed ~pfs ~gamma t low chunk =
    let range_eq = Range.equal ~pfs ~gamma in
    let range_ins = Range.inside ~pfs ~gamma in
    let range = Range.from_low_and_chunk low chunk in
    if range_eq t.span range then Ok (Node.decode ~chunk t.node)
      ...
      | Some (left, right) ->
          if range_ins range left.span then
            get_framed ~pfs ~gamma left low chunk
          else if range_ins range right.span then
            get_framed ~pfs ~gamma right low chunk
          else Error (Unhandled "Children not in partition in get_framed")

  let rec set_framed ~pfs ~gamma t low chunk sval =
    let range_eq = Range.equal ~pfs ~gamma in
    let range_ins = Range.inside ~pfs ~gamma in
    let range = Range.from_low_and_chunk low chunk in
    if range_eq t.span range then
      ...
      | Some (left, right) ->
          if range_ins range left.span then
            let+ left = set_framed ~pfs ~gamma left low chunk sval in
            with_children t ~left ~right
          else if range_ins range right.span then
            let+ right = set_framed ~pfs ~gamma right low chunk sval in
            with_children t ~left ~right
          else Error (Unhandled "Children not in partition in get_framed")
end
...
let drop_perm ~pfs ~gamma t low high new_perm =
  let ( === ) = Gillian.Logic.FOSolver.is_equal ~pfs ~gamma in
  let open Perm.Infix in
  let* cl, hl = get_span t in
  if not (cl === low && hl === high) then
    Error (Unhandled "Modifying permission on range different from object span")
  else
    let* curr_perm = get_perm t in
    if new_perm >% curr_perm then
      Error (InsufficientPermission { required = new_perm; actual = curr_perm })
    else
      let+ root = get_root t in
      Tree { root; span = Range.make cl hl; perm = new_perm }

let free ~pfs ~gamma t low high =
  let ( === ) = Gillian.Logic.FOSolver.is_equal ~pfs ~gamma in
  let open Perm.Infix in
  let* cl, hl = get_span t in
  if not (cl === low && hl === high) then
    Error (Unhandled "Freeing only part of an object")
  else
    let* curr_perm = get_perm t in
    if Freeable >% curr_perm then
      Error (InsufficientPermission { required = Freeable; actual = curr_perm })
    else Ok Freed

let get ~pfs ~gamma t low chunk =
  let inside = Range.inside ~pfs ~gamma in
  let range = Range.from_low_and_chunk low chunk in
  let* span = get_span t in
  if not (inside range span) then Error BufferOverrun
  else
    let* root = get_root t in
    let* root_framed = Tree.frame_single ~pfs ~gamma root low chunk in
    let* value = Tree.get_framed ~pfs ~gamma root_framed low chunk in
    let* wroot = with_root t root_framed in
    Ok (value, wroot)

let set ~pfs ~gamma t low chunk sval =
  let inside = Range.inside ~pfs ~gamma in
  let range = Range.from_low_and_chunk low chunk in
  let* span = get_span t in
  if not (inside range span) then Error BufferOverrun
    ...
    let* root = get_root t in
    let* root_framed = Tree.frame_single ~pfs ~gamma root low chunk in
    let* root_set = Tree.set_framed ~pfs ~gamma root_framed low chunk sval in
