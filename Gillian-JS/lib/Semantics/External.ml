...
module M = struct
  type annot = Jsil_syntax.JAnnot.t
  ...
  struct
              ...
              | Some (String code) -> (
                  ...
                  let opt_proc_eval =
                    try
                      ...
                      Ok
                        (JS2JSIL_Compiler.js2jsil_eval prog cur_proc_id
                           strictness e_js)
                    with
                  ...
                  | Ok proc_eval ->
                      ...
                      let args =
                        Array.make prmlen (Val.from_literal Undefined)
                      in
                      let _ =
                        List.iteri
                  ...
                  | Error error_type -> (
                      ...
                      | _, None ->
                          raise
                            (Failure
                               "Eval triggered an error, but no error label \
                                was provided")
                      | _, _ ->
    ...
    *)
    let execute_function_constructor prog state cs i x v_args j =
      ...
      let params : Literal.t =
        Option.get (Val.to_literal (List.nth v_args 0))
      in
              ...
              | Script (_, [ e ]) -> (
                  ...
                  | JS_Parser.Syntax.Function
                    ...
                    ->
                      ...
                      let params =
                        LList (List.map (fun x -> String x) params)
                      in
